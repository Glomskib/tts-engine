...
## Last refreshed: 2026-05-16 6:28pm ET
...
- **FlashFlow:** deploy pipe is healthy. Local/origin/prod are `1213507` before this docs refresh. This public-doc update will create a newer FlashFlow health SHA, so always verify `https://flashflowai.com/api/health` matches `git rev-parse --short HEAD` after each push.
...
- FlashFlow CRON_SECRET final redeploy is live at `1213507`; `https://flashflowai.com/api/health` matched local/origin before this docs refresh.
