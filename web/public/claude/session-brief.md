...
- Refreshed: `2026-06-07 22:12 EDT`
- Mission Control: prod `7aab3ef`, local/origin `7aab3ef` (in sync)
- FlashFlow pre-publish: prod `6c3e679`, local/origin `6c3e679` (in sync)
...
## Last refreshed: 2026-06-07 22:12 EDT

## Current deploy truth at snapshot time

- **Mission Control:** live match. At snapshot time, local/origin/prod were `7aab3ef` and production health reported `7aab3ef`.
- **FlashFlow:** deploy pipe is healthy. At snapshot time before this public brief was republished, local/origin/prod were `6c3e679` and production health reported `6c3e679`. Because this brief is hosted by FlashFlow, every public context refresh creates the next SHA; the updater verifies final post-publish `/api/health` before exiting.
...
- **MMM hub:** app deploy is healthy on `mmm-hub.vercel.app`. Local head is `0a2ffeb` and the alternate health URL reports `unknown`. `https://makingmilesmatter.org/api/health` still reports `7aab3ef`, so the primary domain/path is still routed to the wrong app until Brandon approves a routing fix.
...
- **MMM primary domain is still not routed to the hub.** `https://mmm-hub.vercel.app/api/health` reports `unknown`, but `https://makingmilesmatter.org/api/health` reports `7aab3ef`. Do not change aliases or DNS without Brandon confirming.
