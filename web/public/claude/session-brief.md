...
## Last refreshed: 2026-05-15 ~3:50am ET

## Current deploy truth

- **Mission Control:** live match. Local/origin/prod are `9700687`. MC deploy pipe is working.
- **FlashFlow:** local has launch-week polish (`9b72ab2`), SEO verification env slots (`37941e1`), and this brief refresh queued for deploy. After push, verify the exact live SHA with `https://flashflowai.com/api/health`.
...
- FlashFlow launch-week create polish is ready to deploy: `/create` now has opt-in B-roll/music toggles defaulting off, the cooking page has friendlier progress/copy/share/failure states, homepage auth redirect was relaxed, public privacy copy uses generic vendor categories, and layout metadata supports Google Search Console + Bing verification through env vars.
