...
async function run(targetDay: string) {
  ...
  // ── Tool usage events (HeyGen, ElevenLabs, etc.) ──
  let toolUsageSection = '_No tool usage data._';
  let toolUsageTotalUsd = 0;
  try {
    const nextDay = new Date(new Date(targetDay).getTime() + 86400000).toISOString().slice(0, 10);
    const { data: toolRows } = await supabase
      .from('tool_usage_events')
      .select('tool_name, cost_usd, success, metadata')
      .gte('created_at', targetDay + 'T00:00:00Z')
      .lt('created_at', nextDay + 'T00:00:00Z');

    if (toolRows && toolRows.length > 0) {
      const toolMap = new Map<string, { cost: number; calls: number; errors: number }>();
      for (const r of toolRows) {
        const entry = toolMap.get(r.tool_name) ?? { cost: 0, calls: 0, errors: 0 };
        entry.cost += Number(r.cost_usd ?? 0);
        entry.calls += 1;
        if (r.success === false) entry.errors += 1;
        toolMap.set(r.tool_name, entry);
      }
      toolUsageTotalUsd = [...toolMap.values()].reduce((s, v) => s + v.cost, 0);

      const lines = [...toolMap.entries()]
        .sort((a, b) => b[1].cost - a[1].cost)
        .map(([name, d]) => `| ${name} | ${d.calls} | ${d.errors} | ${fmt(d.cost)} |`);
      toolUsageSection = `| Tool | Calls | Errors | Cost |\n|------|-------|--------|------|\n${lines.join('\n')}`;

      // HeyGen daily threshold alert ($10 default, configurable)
      const heygenData = toolMap.get('heygen');
      const heygenDailyThreshold = Number(process.env.HEYGEN_DAILY_ALERT_USD) || 10;
      if (heygenData && heygenData.cost > heygenDailyThreshold) {
        alerts.push(
          `**HEYGEN THRESHOLD** Daily HeyGen spend ${fmt(heygenData.cost)} exceeds $${heygenDailyThreshold} threshold (${heygenData.calls} renders)`
        );
      }
    }
  } catch {
...
| Metric | Value |
|--------|-------|
| LLM cost | ${fmt(todayTotal)} |
| Tool cost (HeyGen, etc.) | ${fmt(toolUsageTotalUsd)} |
| **Combined cost** | **${fmt(todayTotal + toolUsageTotalUsd)}** |
| Total LLM calls | ${fmtInt(todayCalls)} |
| Input tokens | ${fmtInt(todayInputTokens)} |
| Output tokens | ${fmtInt(todayOutputTokens)} |
| MTD LLM cost | ${fmt(mtdTotal)} |
...
run(targetDay).then(result => {
  ...
  try {
    ...
    captureRouteError(err instanceof Error ? err : new Error(String(err)), {
      route: 'scripts/finops/daily-report',
      feature: 'finops',
    });
  } catch { /* Sentry unavailable in script context — non-fatal */ }
