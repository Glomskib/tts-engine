...
CREATE TABLE IF NOT EXISTS public.tiktok_login_connections (
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_tiktok_login_conn_user ON public.tiktok_login_connections(user_id);

-- Status index for queries
CREATE INDEX IF NOT EXISTS idx_tiktok_login_conn_status ON public.tiktok_login_connections(status);

-- RLS
ALTER TABLE public.tiktok_login_connections ENABLE ROW LEVEL SECURITY;

-- Users can view their own connection
DROP POLICY IF EXISTS "Users can view own login connection" ON public.tiktok_login_connections;
...
DROP POLICY IF EXISTS "Users can insert own login connection" ON public.tiktok_login_connections;
...
DROP POLICY IF EXISTS "Users can update own login connection" ON public.tiktok_login_connections;
...
DROP POLICY IF EXISTS "Users can delete own login connection" ON public.tiktok_login_connections;
