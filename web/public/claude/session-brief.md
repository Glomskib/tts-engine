...
- Refreshed: `2026-06-11 23:23 EDT`
- Mission Control: prod `6923777`, local/origin `f6d6ecd` (attention (6923777 vs f6d6ecd))
- FlashFlow pre-publish: prod `3b0a070`, local/origin `3b0a070` (in sync)
...
## Last refreshed: 2026-06-11 23:23 EDT

## Current deploy truth at snapshot time

- **Mission Control:** live match. At snapshot time, local/origin/prod were `f6d6ecd` and production health reported `6923777`.
- **FlashFlow:** deploy pipe is healthy. At snapshot time before this public brief was republished, local/origin/prod were `3b0a070` and production health reported `3b0a070`. Because this brief is hosted by FlashFlow, every public context refresh creates the next SHA; the updater verifies final post-publish `/api/health` before exiting.
...
- **MMM hub:** app deploy is healthy on `mmm-hub.vercel.app`. Local head is `0a2ffeb` and the alternate health URL reports `unknown`. `https://makingmilesmatter.org/api/health` still reports `6923777`, so the primary domain/path is still routed to the wrong app until Brandon approves a routing fix.
...
- **MMM primary domain is still not routed to the hub.** `https://mmm-hub.vercel.app/api/health` reports `unknown`, but `https://makingmilesmatter.org/api/health` reports `6923777`. Do not change aliases or DNS without Brandon confirming.
