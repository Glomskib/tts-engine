...
- Refreshed: `2026-06-04 19:23 EDT`
- Mission Control: prod `486f9c9`, local/origin `486f9c9` (in sync)
- FlashFlow pre-publish: prod `1288109`, local/origin `1288109` (in sync)
...
## Last refreshed: 2026-06-04 19:23 EDT

## Current deploy truth at snapshot time

- **Mission Control:** live match. At snapshot time, local/origin/prod were `486f9c9` and production health reported `486f9c9`.
- **FlashFlow:** deploy pipe is healthy. At snapshot time before this public brief was republished, local/origin/prod were `1288109` and production health reported `1288109`. Because this brief is hosted by FlashFlow, every public context refresh creates the next SHA; the updater verifies final post-publish `/api/health` before exiting.
...
- **MMM hub:** app deploy is healthy on `mmm-hub.vercel.app`. Local head is `0a2ffeb` and the alternate health URL reports `unknown`. `https://makingmilesmatter.org/api/health` still reports `486f9c9`, so the primary domain/path is still routed to the wrong app until Brandon approves a routing fix.
...
- **MMM primary domain is still not routed to the hub.** `https://mmm-hub.vercel.app/api/health` reports `unknown`, but `https://makingmilesmatter.org/api/health` reports `486f9c9`. Do not change aliases or DNS without Brandon confirming.
