...
function getStatusPill(status: string) {
  switch (status) {
    ...
    case 'needs_edit':
      return { label: 'In editing', classes: 'bg-blue-500/15 text-blue-400 border-blue-500/20' };
    case 'ready_to_post':
      return { label: 'Ready to post', classes: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/20' };
    case 'posted':
  ...
  if (assignments.length === 0) {
    return (
      <div className="bg-zinc-900/50 rounded-xl border border-white/10 p-6">
        <h2 className="text-lg font-semibold text-white mb-3">Up next today</h2>
        <div className="text-center py-6">
          <p className="text-zinc-500 text-sm">Nothing in your queue yet. Start a video and it'll show up here.</p>
          <Link
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-semibold text-white">Up next today</h2>
        <Link
          href="/admin/pipeline"
