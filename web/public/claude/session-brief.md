...
## Last refreshed: 2026-05-10 ~11:15pm ET
...
- TCG operator ledger allocation core is built in the isolated BuybackOS worktree `~/.config/superpowers/worktrees/buybackos/tcg-operator-ledger`, branch `codex/tcg-operator-ledger`. Local commits: `d295eae` for the allocation engine/test/migration, `d121f8c` for BuybackOS baseline type/lint/build cleanup, and `c956def` for the `/dashboard/tcg` operator dashboard route. The dashboard has member balance metrics, committed allocation metrics, group-buy and member tables, a migration-pending fallback, sidebar navigation, and a focused dashboard aggregation test. The worktree now passes `npx tsc --noEmit --pretty false`, `npm run lint` with warnings only, `npm run test:tcg-allocation`, `npm run test:tcg-dashboard`, and `NEXT_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321 NEXT_PUBLIC_SUPABASE_ANON_KEY=dummy-anon-key npm run build`. Nothing has been pushed, deployed, migrated, priced, or made public.
...
- **TCG Buying Group:** allocation core, initial schema, BuybackOS baseline cleanup, and `/dashboard/tcg` operator read view are now built in an isolated worktree. Next unblocked build: add Brandon-only server actions for members, deposits, supplier buys, and allocation posting, then reconcile BuybackOS main's dirty/ahead local changes with `codex/tcg-operator-ledger`. Do not apply the migration, enable live Stripe pricing, or open public registration without Brandon approval.
