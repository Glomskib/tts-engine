...
## Last refreshed: 2026-05-15 ~2:35am ET

## Current deploy truth

- **Mission Control:** live match. Local/origin/prod are `9700687`. MC deploy pipe is working.
- **FlashFlow:** live match. Local/origin/prod are `32a1c9a` before this brief update; after any brief commit, verify the new exact SHA with `https://flashflowai.com/api/health`.
...
- TCG operator ledger is built and locally migration-validated in the isolated BuybackOS worktree `~/.config/superpowers/worktrees/buybackos/tcg-operator-ledger`, branch `codex/tcg-operator-ledger`. Local commits: `d295eae` allocation engine/test/migration, `d121f8c` BuybackOS baseline type/lint/build cleanup, `c956def` `/dashboard/tcg`, `eeaecdb` Brandon-only write actions, `a11f2ce` transaction-safe `post_tcg_group_buy_allocations` RPC, and `33f38da` local Supabase validation. `33f38da` also pulls in missing local migration files `00020`/`00021`, fixes `00022` to use direct `set_updated_at()` triggers instead of the dropped `attach_updated_at_trigger()` helper, and adds rollback-only `npm run smoke:tcg-rpc:local`. Local Supabase now applies through `20260511161525`, `supabase db lint --local --fail-on error` reports no schema errors, and the authenticated RPC smoke inserts allocation + ledger + allocated status inside a transaction and rolls back. The worktree passes `npm run test:tcg-rpc`, `npm run test:tcg-actions`, `npm run test:tcg-allocation`, `npm run test:tcg-dashboard`, `npx tsc --noEmit --pretty false`, `npm run lint` with warnings only, and `NEXT_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321 NEXT_PUBLIC_SUPABASE_ANON_KEY=dummy-anon-key npm run build`. Nothing has been pushed to BuybackOS, deployed, migrated to production, priced, or made public.
- TCG integration readiness checker added at `~/Documents/Codex/2026-05-09/familiarize-yourself-with-all-code-on/tools/check_buybackos_tcg_integration.py`. Current run: TCG worktree is clean at `33f38da`; BuybackOS main is `22` commits ahead of origin with `15` modified and `16` untracked files; a PR from TCG to `origin/main` would include `28` commits. The real overlaps are `src/app/dashboard/inspections/[id]/page.tsx`, `src/app/dashboard/payouts/[id]/page.tsx`, `src/app/dashboard/quotes/[id]/page.tsx`, and `src/lib/modules/inspections/actions.ts`; duplicated migrations `00020` and `00021` are identical.
...
- BuybackOS base stack: decide whether the 22 committed local main changes should be pushed/PR'd before TCG.
