...
## Last refreshed: 2026-05-10 ~11:46pm ET
...
- TCG operator ledger is built in the isolated BuybackOS worktree `~/.config/superpowers/worktrees/buybackos/tcg-operator-ledger`, branch `codex/tcg-operator-ledger`. Local commits: `d295eae` for the allocation engine/test/migration, `d121f8c` for BuybackOS baseline type/lint/build cleanup, `c956def` for the `/dashboard/tcg` operator dashboard route, and `eeaecdb` for Brandon-only server actions covering members, deposits, supplier invoices, group buys, and allocation posting. The dashboard has member balance metrics, committed allocation metrics, group-buy and member tables, a migration-pending fallback, sidebar navigation, and focused aggregation/action tests. The worktree now passes `npm run test:tcg-actions`, `npm run test:tcg-allocation`, `npm run test:tcg-dashboard`, `npx tsc --noEmit --pretty false`, `npm run lint` with warnings only, and `NEXT_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321 NEXT_PUBLIC_SUPABASE_ANON_KEY=dummy-anon-key npm run build`. Nothing has been pushed, deployed, migrated, priced, or made public. Next production hardening: move allocation posting into a DB transaction/RPC before live use.
...
- **TCG Buying Group:** allocation core, initial schema, BuybackOS baseline cleanup, `/dashboard/tcg` operator read view, and Brandon-only write actions are now built in an isolated worktree. Next unblocked build: make allocation posting transaction-safe with a Supabase RPC, validate/apply the migration in a controlled environment, then reconcile BuybackOS main's dirty/ahead local changes with `codex/tcg-operator-ledger`. Do not apply production migrations, enable live Stripe pricing, or open public registration without Brandon approval.
