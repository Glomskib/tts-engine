...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  if (!target_status || !VALID_STATUSES.includes(target_status as RecordingStatus)) {
    return createApiErrorResponse("BAD_REQUEST", `target_status must be one of: ${VALID_STATUSES.join(", ")}`, 400, correlationId);
  }
  ...
  try {
    ...
    if (updateError) {
      ...
      return createApiErrorResponse("DB_ERROR", "Failed to update video status", 500, correlationId);
    }
