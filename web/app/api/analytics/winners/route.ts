...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const winnersWithViews = allWinners.filter(w => w.views && w.views > 0);
    ...
    const avgWinnerViews = winnersWithViews.length > 0
      ? Math.round(winnersWithViews.reduce((sum, w) => sum + (w.views || 0), 0) / winnersWithViews.length)
      : 0;
    const avgWinnerEngagement = winnersWithEngagement.length > 0
      ...
      : 0;
    const totalViews = allWinners.reduce((sum, w) => sum + (w.views || 0), 0);
    ...
    allWinners.forEach(w => {
      if (w.hook_type) {
        ...
        hookTypeCounts.set(w.hook_type, {
          ...
          totalViews: current.totalViews + (w.views || 0),
        });
      }
    });
    ...
    // Video length stats
    const lengths = allWinners
      .filter(w => w.video_length_seconds && w.video_length_seconds > 0)
      .map(w => w.video_length_seconds!);
    ...
    if (lengths.length >= 3) {
      ...
      videoLengths = {
        ...
        avgWinning: Math.round(lengths.reduce((a, b) => a + b, 0) / lengths.length),
        sweetSpot: {
          min: sorted[q1Index],
          max: sorted[q3Index],
        },
...
async function fetchWinners(userId: string, startDate?: Date | null): Promise<WinnerRow[]> {
  let query = supabaseAdmin
    .from('winners_bank')
    .select('id, views, engagement_rate, hook_type, content_format, video_length_seconds, created_at, ai_analysis')
    .eq('user_id', userId)
    .eq('is_active', true);
...
function calculateEngagementTrends(winners: WinnerRow[]): EngagementTrend[] {
  ...
  winners.forEach(w => {
    ...
    if (w.views) current.views.push(w.views);
