/**
 * POST /api/admin/mmm/agent/draft-post
 *
 * Body: { event_slug: 'fff-2026' | 'hhh-2026', post_type: SocialPostType, extra_context?: string }
 *
 * Calls Bolt/Miles to draft a social post and persists it to marketing_posts
 * with status='cancelled' + meta.requires_approval=true. Owner-gated.
 */
...
import { draftSocialPost } from '@/lib/command-center/mmm/agent-loop';
...
export async function POST(request: Request) {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return createApiErrorResponse(
      'AI_ERROR',
      'ANTHROPIC_API_KEY is not configured. Set it in .env.local and restart dev to enable Bolt/Miles drafts.',
      503,
      correlationId,
    );
  }
  ...
  try {
    const result = await draftSocialPost(parsed.data);
