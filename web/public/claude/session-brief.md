...
## Last refreshed: 2026-05-11 ~12:21pm ET

## Current deploy truth

- **Mission Control:** live match. Local/origin/prod are `a3ad9c2`. MC deploy pipe is working.
...
- **MMM hub:** `mmm-hub.vercel.app` is live at `73814f5` after a prebuilt Vercel deploy. `makingmilesmatter.org` still serves a tiny JavaScript redirect to `/lander`, so the primary domain is not yet pointed at the Vercel app.
...
- **MMM primary domain is still not routed to the hub.** `https://mmm-hub.vercel.app/api/health` reports `73814f5`, but `https://makingmilesmatter.org/` returns a 114-byte redirect shell to `/lander`.
...
- TCG operator ledger is built in the isolated BuybackOS worktree `~/.config/superpowers/worktrees/buybackos/tcg-operator-ledger`, branch `codex/tcg-operator-ledger`. Local commits: `d295eae` for the allocation engine/test/migration, `d121f8c` for BuybackOS baseline type/lint/build cleanup, `c956def` for the `/dashboard/tcg` operator dashboard route, `eeaecdb` for Brandon-only server actions covering members/deposits/supplier invoices/group buys/allocation posting, and `a11f2ce` for transaction-safe allocation posting through `post_tcg_group_buy_allocations`. The RPC locks the group buy row, validates allocation and ledger payloads, inserts allocations and ledger debits, updates group-buy status in one database transaction, grants only narrow authenticated Data API access, and keeps RLS as `SECURITY INVOKER`. The worktree now passes `npm run test:tcg-rpc`, `npm run test:tcg-actions`, `npm run test:tcg-allocation`, `npm run test:tcg-dashboard`, `npx tsc --noEmit --pretty false`, `npm run lint` with warnings only, and `NEXT_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321 NEXT_PUBLIC_SUPABASE_ANON_KEY=dummy-anon-key npm run build`. `supabase db lint --local --fail-on error` could not run because local Postgres at `127.0.0.1:54322` is not running. Nothing has been pushed to BuybackOS, deployed, migrated, priced, or made public.
...
- **TCG Buying Group:** allocation core, initial schema, BuybackOS baseline cleanup, `/dashboard/tcg` operator read view, Brandon-only write actions, and transaction-safe allocation RPC are now built in an isolated worktree. Next unblocked build: start/repair local Supabase, run the TCG migrations and RPC smoke test against a local database, then reconcile BuybackOS main's dirty/ahead local changes with `codex/tcg-operator-ledger`. Do not apply production migrations, enable live Stripe pricing, or open public registration without Brandon approval.
